...
$storyQuery = "SELECT * FROM success_stories WHERE story_id = ?";
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errors)) {
        ...
        if (empty($errors)) {
            // Update the story
            $query = "UPDATE success_stories SET
                     title = ?,
                     content = ?,
                     author_name = ?,
                     author_image = ?,
    ...
    <div class="row">
        ...
        <main class="col-md-9 ms-sm-auto col-lg-10 px-md-4">
            <div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-3 border-bottom">
                <h1 class="h2">Edit Success Story</h1>
...
<script>
    $(document).ready(function() {
        ...
        $('#remove_image').change(function() {
            if ($(this).is(':checked')) {
                $('#author_image').prop('disabled', true);
            } else {
                $('#author_image').prop('disabled', false);
            }
        });
    });
</script>