...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_POST['add_category'])) {
        ...
        if (empty($errors)) {
            ...
            $check = $conn->prepare("SELECT category_id FROM blog_categories WHERE slug = ?");
        ...
        if (empty($errors)) {
            ...
            $check = $conn->prepare("SELECT category_id FROM blog_categories WHERE slug = ? AND category_id != ?");
            ...
            if ($check->num_rows > 0) {
                ...
                $stmt = $conn->prepare("UPDATE blog_categories SET name = ?, slug = ?, description = ? WHERE category_id = ?");
        ...
        $check = $conn->prepare("SELECT post_id FROM blog_post_categories WHERE category_id = ? LIMIT 1");
        $check->bind_param("i", $category_id);
        $check->execute();
        $check->store_result();
        
        if ($check->num_rows > 0) {
            $errors[] = "Cannot delete category - it is assigned to one or more posts";
        } else {
